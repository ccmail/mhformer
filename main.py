...
if opt.train:
    # 设置log
    log_name = os.path.join(opt.log_path, 'train.log')
else:
    log_name = os.path.join(opt.log_path, 'test.log')

log = Logger(log_name, level=opt.log_level)

# 设置gpu, 设置哪些卡可视
os.environ["CUDA_VISIBLE_DEVICES"] = opt.gpu
# gpu_nums = len(opt.gpu.split(','))

# 可视卡从0开始编号, 编号恰好为range(gpu_nums)
# device_ids = list(range(gpu_nums))

log.logger.info("可视显卡的物理编号为[%s]", os.environ["CUDA_VISIBLE_DEVICES"])
# log.logger.info("可视显卡的虚拟编号为[%s]", str(device_ids))
...
if __name__ == '__main__':
    ...
    if opt.train:
        train_data = Fusion(opt=opt, train=True,
                            dataset=dataset, root_path=root_path)
        train_dataloader = torch.utils.data.DataLoader(train_data, batch_size=opt.batch_size,
                                                       shuffle=True, num_workers=int(opt.workers), pin_memory=True)

    test_data = Fusion(opt=opt, train=False,
                       dataset=dataset, root_path=root_path)
    ...
    if opt.previous_dir != '':
        model_paths = sorted(
            glob.glob(os.path.join(opt.previous_dir, '*.pth')))
        ...
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in pre_dict.items()
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, 'min', factor=0.317, patience=5, verbose=True)

    for epoch in range(1, opt.nepoch):
        if opt.train:
            loss = train(opt, actions, train_dataloader,
        ...
        if opt.train == 0:
            log.logger.info('p1: %.2f, p2: %.2f' % (p1, p2))
            break
        else:
            log.logger.info('epoch: %d, lr: %.7f, loss: %.4f, p1: %.2f, p2: %.2f' % (
                epoch, lr, loss, p1, p2))
            log.logger.info('e: %d, lr: %.7f, loss: %.4f, p1: %.2f, p2: %.2f' %
                            (epoch, lr, loss, p1, p2))
    ...
    log.logger.info(opt.checkpoint)
