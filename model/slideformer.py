...
class SlideFormer(nn.Module):
    def __init__(self, args):
        super().__init__()
        # 配置部分参考mhformer即可

        # norm这里是以帧数视为维度, nn.LayerNorm是对三维向量的最后一个维度进行处理, 这里考虑将其设置为滑动窗口的大小
        self.norm = nn.LayerNorm(args.frames)

        # 这里使用Transformer对窗口内数据进行MHSA
        # depth: tranformerBlock个数, embed_dim是attention的dim,
        self.transformer = Transformer_encoder(
            depth=4, embed_dim=args.frames, mlp_hidden_dim=args.frames*2, length=2*args.n_joints, h=9
        )
    ...
    def forward(self, x):
        # 这里需要对输入的x进行调整, 假设我的滑窗大小为10, 需要将x分解为f-10+1个子序列
