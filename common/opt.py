...
class opts():
    ...
    def init(self):
        ...
        self.parser.add_argument(
            '--data_augmentation', type=bool, default=True)
        self.parser.add_argument(
            '--reverse_augmentation', type=bool, default=False)
        self.parser.add_argument(
            '--test_augmentation', type=bool, default=True)
        ...
        # 这里改GPU
        self.parser.add_argument('--gpu', default='0,1,5', type=str, help='')
        self.parser.add_argument('--log_level', default='info', type=str,
                                 help='请输入打印的日志等级, 从"debug,info,warning,error,crit"中选择, 从左到右log等级依次升高')
        ...
        self.parser.add_argument(
            '--batch_size', type=int, default=256, help='can be changed depending on your machine')
        ...
        self.parser.add_argument('-lrd', '--lr_decay',
    ...
    def parse(self):
        ...
        if self.opt.train:
            ...
            self.opt.log_path = 'checkpoint/log/' + logtime + '%d' % (self.opt.frames)
            ...
            # 输出配置文件
            file_name = os.path.join(self.opt.log_path, 'opt.txt')
