...
class Fusion(data.Dataset):
    def __init__(self, opt, dataset, root_path, train=True):
        ...
        if self.train:
            ...
            log.logger.info('Training on {} frames'.format(
            ...
            log.logger.info('Testing on {} frames'.format(
                self.generator.num_frames()))
    ...
    def prepare_data(self, dataset, folder_list):
        ...
        keypoints = np.load(self.root_path + 'data_2d_' + self.data_type +
                            '_' + self.keypoints_name + '.npz', allow_pickle=True)
        keypoints_symmetry = keypoints['metadata'].item()['keypoints_symmetry']

        self.kps_left, self.kps_right = list(
            keypoints_symmetry[0]), list(keypoints_symmetry[1])
        self.joints_left, self.joints_right = list(
            dataset.skeleton().joints_left()), list(dataset.skeleton().joints_right())
            ...
            for action in self.keypoints[subject].keys():
                ...
                if subject in dataset.cameras():
                    ...
                    for i, cam in enumerate(cams):
                        if 'intrinsic' in cam:
                            out_camera_params[(
                                subject, action, i)] = cam['intrinsic']
